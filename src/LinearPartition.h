...
public:
    ...
    pf_type m_energy;
    pf_type m_logPartitionCoef;

    BeamCKYParser(bool useVienna=true,
                  int beam_size=100,
...
private:
    ...
    // inlined methods
    pf_type Fast_LogExpPlusOne(pf_type x);
    void Fast_LogPlusEquals (pf_type &x, pf_type y);
    pf_type Fast_Exp(pf_type x);
    ...
    template <typename pf_type, typename value_type>
    inline pf_type BeamCKYParser<pf_type, value_type>::Fast_LogExpPlusOne(pf_type x){
    ...
    template <typename pf_type, typename value_type>
    inline void BeamCKYParser<pf_type, value_type>::Fast_LogPlusEquals (pf_type &x, pf_type y)
    {
        if (x < y) std::swap (x, y);
    ...
    template <typename pf_type, typename value_type>
    inline pf_type BeamCKYParser<pf_type, value_type>::Fast_Exp(pf_type x)
    {
