...
void BeamCKYParser<pf_type, value_type>::output_to_file(string file_name, const char * type) {
    if(!file_name.empty()) {
...
template <typename pf_type, typename value_type>
void BeamCKYParser<pf_type, value_type>::output_to_file_MEA_threshknot_bpseq(string file_name, const char * type, map<int,int>& pairs, string & seq) {
...
template <typename pf_type, typename value_type>
void BeamCKYParser<pf_type, value_type>::cal_PairProb(State& viterbi) {
...
template <typename pf_type, typename value_type>
string BeamCKYParser<pf_type, value_type>::back_trace(const int i, const int j, const vector<vector<int> >& back_pointer){
...
template <typename pf_type, typename value_type>
map<int, int> BeamCKYParser<pf_type, value_type>::get_pairs(string & structure){
    map<int, int> pairs;
    stack<int> s;
    int index = 1;
    int pre_index = 0;
...
template <typename pf_type, typename value_type>
void BeamCKYParser<pf_type, value_type>::ThreshKnot(string & seq){
...
template <typename pf_type, typename value_type>
void BeamCKYParser<pf_type, value_type>::PairProb_MEA(string & seq) {
...
template <typename pf_type, typename value_type>
void BeamCKYParser<pf_type, value_type>::outside(vector<int> next_pair[]){
            ...
            for(auto& item : beamstepP) {
                ...
                // 4. C = C + P
                {
                    ...
                    if (k >= 0) {
                        int nuck = nuci_1;
                        int nuck1 = nuci;
                        pf_type external_paired_alpha_plus_beamstepC_beta;
                        if (useVienna) {
